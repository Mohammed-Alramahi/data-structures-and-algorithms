...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  for (let index = 0; index < starWarsArr.length; index++){   
    return starWarsArr.sort((a,b)=>{
    return b.height - a.height;
  });
    
  }
    return starWarsArr.name[0];  
}
...
const listFoods = (recipe) => {
  ...
  for(let i = 0; i < recipe.ingredients.length; i++){
    let firstArr = recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(" ")+1);
    result.push(firstArr.slice(firstArr.indexOf(" ")+1));
  }
  return result;
};
