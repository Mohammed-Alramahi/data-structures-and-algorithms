...
Writae a function named addQuestion that, given an array of strings, uses map to return a new array containing each string followed by a question mark character.
------------------------------------------------------------------------------------------------ */
...
const extractStats = (arr) => {
  ...
  let newArr=arr.map(item=>({
   name:item.stat.name,
   total:item.baseStat+item.effort
  }));
  return newArr;
};
